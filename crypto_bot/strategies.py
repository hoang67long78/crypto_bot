...
class Strategy:
    ...
    def _open_position(self, signal_result: int):
        ...
        elif self.trade_now == 1:
            trade_size = self.client.get_trade_size(self.contract, self.candles[-1].close, self.balance_pct)*6
...
class TechnicalStrategy(Strategy):
    ...
    def _macd(self) -> Tuple[float, float]:
        ...
        macd_histogram_2 = macd_line.iloc[-2] - macd_signal.iloc[-2]

        macd_histogram_3 = macd_line.iloc[-3] - macd_signal.iloc[-3]

        return macd_histogram_2, macd_histogram_3

        #return macd_line.iloc[-2], macd_signal.iloc[-2]

    def _check_signal_RSI(self):

        rsi = self._rsi()

        if rsi < 30:
            self.side_RSI = 1
            return self.side_RSI
        elif rsi > 70:
            self.side_RSI = -1
            return self.side_RSI
        else:
    ...
    def _check_signal(self):
        ...
        macd_histogram_2, macd_histogram_3 = self._macd()

        if self.side_RSI == 1 and macd_histogram_2 > macd_histogram_3:
            self.side_RSI = 0
            self.side_now = 1
            return 1

        elif self.side_RSI == -1 and macd_histogram_2 < macd_histogram_3:
            self.side_RSI = 0
            self.side_now = -1
            return -1
    ...
    def check_trade(self, tick_type: str):
        ...
        if self.ongoing_position == False:
            ...
            if tick_type == "new_candle":
                ...
                elif self.trade_now == 0:
                    if self.side_RSI == 0:
                        self._check_signal_RSI()
                    self.trade_on = self._check_signal()
            ...
            if self.trade_now == 1:
                ...
                elif self.side_now == -1:
                    self.trade_on = -1

            if  self.trade_on != 0:
                signal_result = self.trade_on
